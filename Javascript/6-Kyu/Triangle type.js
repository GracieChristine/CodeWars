...
function triangleType(first, second, third) { // inputs are side lengths
  var inputArray = [first, second, third].sort(function(a, b) {
    return a - b;
  });
  var a = inputArray[0];
  var b = inputArray[1];
  ...
  if ((a * a) + (b * b) < (c * c)) { // obtuse triangle
    return 3;
  }
  if ((a * a) + (b * b) > (c * c)) { // acute triangle
    return 1;
  }
}
